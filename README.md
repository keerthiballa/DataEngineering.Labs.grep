...
```
grep -i start transaction_data_daily_event_log_20190129.dat
```
...
```
grep -ic start transaction_data_daily_event_log_20190129.dat
```


Use grep to find all instances where the upload was successful. 
```
grep -i complete transaction_data_daily_event_log_20190129.dat
```
...
```
grep -i complete transaction_data_daily_event_log_20190129.dat | wc -l
```
...
```
grep -in failure transaction_data_daily_event_log_20190129.dat 
```

Upon review, we would like to only view failures with error code SYSOFFLINE or WEAKSIGNAL.

```
grep -in failure transaction_data_daily_event_log_20190129.dat  | egrep 'SYSOFFLINE|WEAKSIGNAL'
```
...
```
grep -E '@\w+(.edu)' users.csv > academia_users.txt
```
